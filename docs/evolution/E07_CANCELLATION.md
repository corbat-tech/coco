...
## E07.n2a · DONE · 2026-09-16

OpenAI con herramientas, Chat Completions y Responses (stream/no stream), exige terminal válido antes de entregar llamadas ejecutables. Texto/start/deltas siguen progresivos; todas las llamadas se validan antes del primer end. CC requiere tool_calls cuando hay llamadas, permite texto sin tools con stop/length y rechaza EOF sin terminal. Responses retiene arguments.done hasta response.completed con status completed; incomplete/failed/error y EOF rechazan sin tools ni retry automático. Output terminal puede completar fragmentos, nunca contradecir argumentos completos.

Repeticiones idénticas por ID se deduplican; IDs distintos con mismos argumentos se conservan. Contradicciones de identidad/argumentos y referencias desconocidas/ambiguas fallan sin elegir el último builder. La revisión detectó conflictos silenciosos por Map.set y por arguments.done; corregidos con regresiones. Watchdog timeout0 respeta desactivación. El transporte heredado de Copilot recibe la misma protección OpenAI; su fixture de éxito ahora incluye terminal realista. No afirma todavía integridad terminal de otros proveedores ni del consumidor.

Fuentes oficiales consultadas2026-09-16: [Chat finish_reason](https://developers.openai.com/api/reference/resources/chat) y [Responses streaming events](https://developers.openai.com/api/reference/resources/responses/streaming-events). La finalización de argumentos no equivale a finalización de respuesta; implementación de Coco deriva de esa distinción. SDK OpenAI6.34.0 y catálogo conservados.

43 contratos terminales independientes y cuatro timeout0 añadidos. Gate focal230 tests; gate amplio356 archivos/7790 correctos,15 omitidos; REPL27; typecheck/lint/format/build correctos. Logs e07n2a-final-contracts/main-final/repl/final-types/final-lint/final-format/build. El test preparado para n2b se excluyó expresamente y no acredita comportamiento Anthropic. /root/file_fixture_update fuente; coordinador fixtures/watchdog e integración; /root/core_audit contratos; /root/baseline_review APPROVED fuente. Sin publicación; rollback por revert.

## E07.n2b · DONE · 2026-09-16

Anthropic con herramientas exige message_stop y stop_reason compatible; las tools requieren tool_use y bloques cerrados con índice correcto. Parser y lote completo se validan antes del primer tool_use_end. EOF, bloques abiertos, argumentos incompletos y conflictos de identidad fallan sin tools ni retry. Progreso text/start/delta y cancelación entre emisiones se conservan; timeout0 desactiva ambos watchdogs. Si no hubo delta, solo se acepta input realmente recibido en el bloque inicial, nunca un objeto inventado.

Revisión detectó bloques nulos que generaban TypeError recuperable: guard mínimo los convierte en ResponseIntegrityError, con tres regresiones de una petición pese a retries3. Fuente [streaming Anthropic](https://platform.claude.com/docs/en/build-with-claude/streaming), consultada2026-09-16: bloques start/delta/stop y final message_stop. SDK0.95.2/modelos conservados.

24 contratos terminales, fixtures fieles al protocolo y prueba SDK HTTP compartida. Gate5 archivos/154 tests, typecheck/lint/format correctos; logs e07n2b-final/final-types/final-lint/final-format. /root/file_fixture_update fuente; coordinador fixtures/shape guards; /root/core_audit contratos; /root/baseline_review APPROVED. Codex/Gemini/Vertex y consumidor siguen pendientes. El usuario solicita detener ampliación, persistir continuación y publicar candidato con alcance actual.
