 ...
 */
function parseNumstatOutput(output: string): FileChange[] {
  ...
  for (const line of output.split("\n").filter(Boolean)) {
    ...
    if (match) {
      ...
      if (filePath) {
        const additions = parseInt(added ?? "0", 10);
        const deletions = parseInt(deleted ?? "0", 10);
        const status: FileChange["status"] =
          additions === 0 ? "deleted" : deletions === 0 && additions > 0 ? "added" : "modified";
 ...
 */
export function filterAutoGenerated(files: FileChange[]): FileChange[] {
