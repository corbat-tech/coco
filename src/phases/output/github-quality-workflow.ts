/**
 * GitHub Actions — Quality Workflow Generator
 *
      ...
      - name: Post quality report as PR comment
        ...
        with:
          script: |
            ...
            const status = report.meetsMinimum ? '✅ PASS' : '❌ FAIL';
            const body = \`## Quality Report — \${status}\\n\\n**Overall score: \${score}/100**\\n\\n> Generated by Corbat-Coco\`;
...
export function formatQualityPRComment(
  ...
  const statusBadge = evaluation.meetsMinimum ? "✅ PASS" : "❌ FAIL";
  const evaluatedAt = evaluation.scores.evaluatedAt.toISOString().slice(0, 10);

  const lines: string[] = [
    `## Quality Check — ${statusBadge}`,
    ``,
  ...
  if (showDimensions) {
    lines.push(`<details>`);
    lines.push(`<summary>📊 Dimension breakdown</summary>`);
    lines.push(``);
  ...
  lines.push(`---`);
  lines.push(`_🥥 Generated by [Corbat-Coco](https://github.com/corbat-tech/coco)_`);
