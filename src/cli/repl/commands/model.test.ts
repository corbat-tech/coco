...
describe("modelCommand", () => {
  ...
  describe("execute with valid model argument", () => {
    it("should change to a known model in same provider", async () => {
      ...
      const result = await modelCommand.execute(["claude-opus-4-8"], mockSession);

      expect(mockSession.config.provider.model).toBe("claude-opus-4-8");
      expect(result).toBe(false);
    });

    it("should display success message", async () => {
      await modelCommand.execute(["claude-opus-4-8"], mockSession);
